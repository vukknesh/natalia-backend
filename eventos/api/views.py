    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if data_final and data_inicial:
            ...
            queryset_list = Evento.objects.filter(user__is_active=True, starting_date__range=[
                                                  data_inicial, data_final]).values('starting_date').distinct()  # filter(user=self.request.user)
            experimental = Experimental.objects.filter(starting_date__range=[
                data_inicial, data_final])
        else:
            queryset_list = Evento.objects.filter(user__is_active=True, starting_date__gte=dt).values('starting_date').distinct()[
                :900]
